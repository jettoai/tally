...
- **돌아가는 세션에 할 일을 시키기.** `tally type claude "<텍스트>" --project <프로젝트명>`은 그
  ...
  때는 큐에 넣고 15분 뒤 만료됩니다. 터미널이 조용하지만 설명할 수 없는 입력이 있으면 즉시
...
- **로컬 중심, 오류 보고는 선택 사항이며 기본값은 꺼짐.** 텔레메트리 없음, 서버 없음. 사용량
  읽기와, 설정에서 「충돌 및 오류 보고서 보내기」를 켠 경우 Sentry로 보내는 오류 보고 외에는
  아무것도 기기 밖으로 나가지 않습니다. 보고에는 스택 트레이스, 앱 및 macOS 버전, Mac 모델,
