#!/bin/bash
# Compiles the CLI's burn-rate account pick (TallyCLI/Snapshot.swift) together with a small
# assertion harness. ResumePrompt is here because the unsupervised launcher in Snapshot.swift
# asks it what to hand the exec'd child (`resumePromptSuppression`).
# assertion harness and runs it. No Xcode test target needed; exits non-zero on failure.
set -euo pipefail
cd "$(dirname "$0")/.."
out=$(mktemp -d)/run
swiftc -o "$out" tests/smartpick/main.swift TallyCLI/Snapshot.swift TallyCLI/AccountPick.swift \
  TallyCLI/ResumePrompt.swift \
  TallyCLI/ProviderExecutable.swift TallyCLI/AccountComfort.swift
"$out"
