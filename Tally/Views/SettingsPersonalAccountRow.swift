...
extension SettingsAccountsView {
    ...
    func reserveRow(_ home: String, underBadge: Bool) -> some View {
        // A capture shows the fixture's figure, and its stepper writes nothing (the marking itself
        ...
        return HStack(alignment: .firstTextBaseline) {
            ...
            Stepper(value: Binding(
                get: { shown },
                set: { LaunchPolicyStore.shared.setReserve(home, $0) }
            ), in: AccountRoles.reserveBounds, step: AccountRoles.reserveStep) { EmptyView() }
                .labelsHidden()
