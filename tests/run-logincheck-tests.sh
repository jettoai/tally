...
# with the spawn it runs on (Tally/Core/CLIRunner.swift) and the renewal knowledge it borrows its
# environment and its ANSI stripping from (Tally/Core/RenewLoginCommand.swift), then runs it. All
# three are Foundation-only, so nothing else comes along; no Xcode test target needed. Exits
# non-zero on failure.
#
