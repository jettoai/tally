...
<p align="center">
  <img src="assets/screenshot-tokens.png" alt="Tally のトークンタブ：選択した期間（今日／7 日／30 日／全期間）のトークン合計を、入力・キャッシュ書込・キャッシュ読込・出力の内訳、Claude と Codex のプロバイダ別の内訳、そしてトークンがどこへ行ったかを示す割合バー付きのプロジェクト別テーブルとともに表示します" width="834">
</p>
...
- **トークン使用量をプロジェクト別に。** どの画面でもヘッダーのスイッチの裏にトークン
  ...
  ワークフローのセッションまで、それが担ったプロジェクトへ遡って集計します。データは CLI
  自身のローカルトランスクリプトから読み取り、増分キャッシュで集計され（新しいデータが
  なければ更新は 1 秒もかかりません）、あなたのマシンから出ることは一切ありません。
...
- **`tally` CLI。** `tally claude [引数…]`、`tally resume`（このディレクトリの最新の会話を
  ...
  次の空き番号の選定も設定ディレクトリの作成もおまかせ。メインアカウントの
  CLAUDE.md/AGENTS.md、skills、hooks、agents、設定、会話履歴はデフォルトで新しい
  ディレクトリへシンボリックリンクされ、ひとつのセットアップを全アカウントで
  使い回せます。`--no-share` でオプトアウトできます）、`tally best-dir <provider>`。
  すべてスクリプトフレンドリーです。
...
しません。読み取り専用で、自分で契約した有料サブスクリプションのみが対象です。
