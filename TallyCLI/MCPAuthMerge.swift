...
// MARK: - Whether a home has to be read at all
...
// write to this gate, and a sibling written in the same second as the record reads as unchanged.
// What that costs is bounded by the two rules either side of it: the comparison is STRICTLY greater,
// so such a sibling is read again on the NEXT launch rather than never, and the merge is add-only
// newest-wins, so a grant that arrives one launch late arrives whole. The worst outcome is a seeding
// that happens at the next launch instead of this one, which is the same outcome as every other
// fail-open path here (MCPAuthSync.swift's header).
//
...
// sibling still had it (the add-only edge the header of this file describes); now it is undone at
...
///
///   - NO RECORD for that home. Either this target has never merged from it, or the record was lost
///     (the app rewrites the state file from its own model, and an older build of it knows no such
///     key). Both mean read: a home that has never merged from its siblings is the case this whole
///     feature exists for, and losing a record may only ever cost a read.
...
/// The key the seeding record lives under in `~/.tally/state.json`. TOP LEVEL, beside the other two
/// blocks of that document that are not launch decisions (`artifactAccount`, `accounts`), and added
/// under the rule they were added under: the document only ever GAINS keys, so a Tally that has
/// never heard of this one reads the file exactly as it did before.
let mcpSeedStateKey = "mcpSeed"

/// What `home` last merged from, out of a decoded state document.
///
/// KEYED BY THE TARGET HOME FIRST and the sibling second, which is the point of the block rather
...
func mcpSeedRecord(in state: [String: Any], for home: String) -> MCPSeedRecord {
    guard let block = state[mcpSeedStateKey] as? [String: Any],
          let mine = block[home] as? [String: Any] else { return [:] }
...
/// The state document with `home`'s record replaced and EVERYTHING ELSE carried through by value.
///
/// The document belongs to the app (`LaunchPolicyStore` publishes it) and holds the pinned account,
/// the launch defaults and the per-account reserves. This is the CLI's only write into it, so what
/// is written is the document AS READ with one key patched, rather than a document rebuilt out of
/// what this build happens to understand: a key added by a newer Tally survives the write.
func stateDocumentSettingMCPSeedRecord(_ state: [String: Any], for home: String,
                                       record: MCPSeedRecord) -> [String: Any] {
    var block = state[mcpSeedStateKey] as? [String: Any] ?? [:]
    block[home] = record.mapValues { $0.timeIntervalSince1970 }
    var next = state
    next[mcpSeedStateKey] = block
    return next
}
