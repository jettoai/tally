...
func runLaunchAtLoginChecks() {
    ...
    check("the login-health preview only modifies demo data",
          CaptureLaunch.modifierKeys.contains("TallyLoginHealthPreview")
            && CaptureLaunch.mayTakeForeground(activeKeys: ["TallyLoginHealthPreview"]))
    check("login-health fixtures with demo data remain a background capture",
          !CaptureLaunch.mayTakeForeground(activeKeys: ["TallyLoginHealthPreview", "TallyDemoData"]))
