...
<p align="center">
  <img src="assets/screenshot-panel.png" alt="Tally のピン留めパネル：プロバイダごとのフリートゲージが 9 つのアカウント（Claude Max が 5 つ、Codex が 4 つ）をまとめ、Claude は 2 本の残量（Fable プールと毎週プールのバー、それぞれペース予測「残り約 4d 12h」と次の時差回復付き）、Codex は毎週プール 1 本（このペースなら持続可能）を表示。アドバイザー行はアカウントのドットで各プロバイダの週あたりの実需（5.8 acct/wk）を示し、ペースが超過するときはアカウントの追加を促します。その下には各アカウント自身のカードが 5 時間セッション、毎週、最上位モデルの窓をリセット時刻、上限接近の警告、そしてランチャーの現在の選択を示す紫色のスマート選択バッジとともに表示します" width="834">
</p>
