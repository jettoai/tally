...
enum AccountSignIn {
    ...
    /// Local status establishes sign-in availability, not a refresh-token expiry date.
    /// The credential deadline has its own notice in `AccountLoginHealthView`.
    static func detailKey(isDormant: Bool) -> String {
        isDormant ? "Not signed in. Click to sign in." : "Signed out. Click to sign in again."
    }
}
