...
- **레이아웃을 카드 한 장씩 원하는 대로.** 푸터의 보기 옵션 카드에서 레이아웃 타일(각
  타일이 만들어지는 배치를 그대로 그려 줍니다)로 열 수를 정하고, 프로바이더를 게이지
  아래로 접고, 카드를 프로바이더별 섹션에 앉힐 수도 있습니다. 섹션 안에서도 드래그로
  순서를 바꿀 수 있습니다. 화면보다 커진 플릿은 밖으로 밀려나는 대신 스크롤됩니다.
- **토큰 사용량을 프로젝트별로.** 모든 화면의 헤더 스위치 뒤에 토큰 뷰가 있습니다:
  ...
  프로젝트로 되짚어 집계합니다. 데이터는 CLI 자체의 로컬 트랜스크립트에서 읽어 증분
  캐시로 집계하며(새 데이터가 없으면 새로고침은 1초도 걸리지 않습니다), 기기 밖으로
  나가지 않습니다.
...
- **앱을 벗어나지 않고 계정 추가.** 설정이 다음 계정의 구성 홈을 준비하고, 메인 계정 설정
  ...
  같은 조용한 로그인을 실행합니다. 브라우저에서 돌아오면 새 카드가 나타납니다.
...
다룹니다. 금액 추정은 하지 않습니다. 읽기 전용이며, 본인이 결제한 구독만 대상으로 합니다.
