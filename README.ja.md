...
- **ログイン状態をひと目で確認。** カードにホバーすると、ログイン中のメールアドレスが
  表示されます。ログアウト済みのアカウントには赤いチップと通知でお知らせします。
  `/login` が必要なセッションには個別の警告が付き、そのセッションを直接開けます。
  Claude のログイン期限を取得できる場合は、期限まで 3 日以内になったときと期限切れ後に
  お知らせします。ログインの更新をクリックすると、公式 CLI がバックグラウンドでログインを
  実行し、ブラウザで承認します（必要に応じてターミナルを表示）。このログイン処理では、
  Tally が認証情報を直接変更することはありません。
...
- **すべての会話をひとつのボードに。** ヘッダーのスイッチの 3 番目の位置、使用状況と
  ...
  Tally がそのセッションのターミナルを前面に出します。おすすめは Ghostty です（Tally は
  Ghostty を中心に作られています）：ジャンプはセッション自身の tty をたどり、正確にその
  タブに着地します。それ以外のターミナルでは今のところアプリを前面に出すまでで、タブ単位の
  ジャンプ対応はロードマップにあります。
...
- **早めのスタート。** 5 時間ウィンドウは、あなたが何かを送って初めて数え始めます。だから
  一日の最初のリセットは席についてから 5 時間後に訪れ、休憩のたびに次のリセットは後ろへ
  ずれていきます。Claude アカウントのウィンドウが閉じているときは、Tally が公式 CLI 経由で
  短いメッセージを一通だけ送って開けます。いちばん安いモデルに固定してあるので、あなたが
  眠っている間も時計は進み、リセットは一日のより早い時間に訪れます。1 アカウントにつき
  5 時間で最大一通、すでに動いているアカウントには手を出さず、任意のサイレント時間帯を
  設定すれば夜通し黙ったままです。認証情報には一切触れず、最初のメッセージを送る前に一度
  だけ知らせ、設定欄にはその日の正直な集計が残ります：何アカウントが開始し、意図的に
  スキップされ、開始できなかったのか。
...
- **動いているセッションに指示を出す。** `tally type claude "<テキスト>" --project <プロジェクト名>`
  ...
  端末が静かでも説明できない入力があるときは即座に拒否します。その Codex セッションで実際の
...
- **Harness ツール。** 設定 > 連携 > Harness から、両方のアシスタントに
  Tally ハーネススキルとオフライン受信箱の通知をまとめてインストール、削除できます。
  その後、現在のプロジェクトやユーザー設定のハーネス適応をどちらのアシスタントにも依頼できます。
  ツールのインストールだけではプロジェクトの規則は変わりません。ネイティブフックの信頼設定は
  別途確認してください。[ハーネスガイド](docs/harness.md)を参照してください。
...
- **公式 CLI 経由で使用量を取得。** 使用量は各プロバイダの**公式 CLI**
  （`claude -p "/usage"` と `codex app-server`）を通じて読み取ります。公式クライアントが
  プロバイダとの認証と認証情報の管理を担い、アカウント検出ではログインの属性を確認します。
  読み取り専用の例外として、Claude のログイン期限を取得するために、Tally は Keychain 内の
  ローカル認証情報ドキュメントを一時的に読み取ります。Keychain 項目が存在しない場合は
  `.credentials.json` から `refreshTokenExpiresAt` を取得します。このリーダーが外部に返すのは
  日付のみで、トークンのログ出力、永続保存、送信、ログインの更新、認証情報の変更は行いません。
...
- **自分のアカウントだけ。** マルチアカウントとは、*あなた自身*が支払い、*あなたの*マシン上に
  ...
  起動するだけです。
- **ローカル中心、エラーレポートは任意で既定オフ。** テレメトリなし、サーバーなし。使用量の
  読み取りと、設定で「クラッシュとエラーのレポートを送信」をオンにした場合に Sentry へ送る
  エラーレポート以外、何もマシンの外へ出ません。レポートに含まれるのはスタックトレース、
  アプリと macOS のバージョン、Mac の機種、言語とタイムゾーン、接続から Sentry が推定する
  おおよその地域で、IP アドレス、アカウント、使用量の数値、ファイルの内容は決して含まれません。同じ場所でいつでもオフにできます。
...
サブエージェント数のフック、クォータ警告のフック、Artifact 公開のガード（公開された
Artifact は公開したアカウント専用なので、ブラウザでサインインしているアカウント以外から
公開しようとする操作を Tally が止め、その行がそのアカウントを指定します）、そして `/tally`
コマンド付きの Claude Code スキル。フックの行はどれも同じ約束を守ります：そのイベントに
すでに登録していたものはそのまま動き続け、削除で外れるのは Tally 自身のエントリだけです。
既存アカウントを共有の一式に載せる行もここにありますが、「すべてインストール」からは
意図的に外してあります。これは config home の間で会話を移動させる操作であり、「全部オンに
する」という意味の一押しが、それも意味するとは限らないからです。
...
**Tally は Keychain にどのようにアクセスしますか？**
使用量は公式 CLI 経由で取得し、アカウント検出では属性のみを確認します。Claude のログイン期限
リーダーは認証情報ドキュメントも読み取りますが、事前確認で Keychain のロックを検出した場合や、
リーダーが信頼されていると確認できない場合はアクセスを省略し、期限は不明のままにします。
確認から読み取りまでの間に Keychain の状態が変わることがあるため、許可ダイアログが表示される
可能性はあります。ログインは引き続き公式 CLI が行い、Tally のログイン処理は認証情報を直接変更しません。
