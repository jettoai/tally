...
的 5 小时会话、每周、旗舰模型额度窗并排呈现，智选会在每次开新会话时，依重置时间（不只看
剩余百分比）挑出当下余量撑最久的账号，并在对话进行中持续接管：额度撞墙时自动切换账号、
...
<p align="center">
  <img src="assets/screenshot-panel.png" alt="Tally 固定面板：八个账号并排（五个 Claude Max、三个 Codex），各自的 5 小时会话、每周、旗舰模型额度窗、重置时间与接近上限警示；紫色智选徽章标出启动器会选中的账号，标题栏摘要显示启动默认值" width="560">
</p>
...
- **每个窗口自己的重置时间。** 点任何重置文字，全部在「2d 4h 后重置」与「07/18 20:00 重置」
  之间切换。
- **Codex 额度重置存量，看得见。** 累积的额度重置会直接显示在卡片上（「3 枚额度重置可兑换」），
  让你在撞墙前就知道自己还有几条退路；要不要兑换由你自己在官方 CLI 里决定。
...
- **每个 provider 三种模式。** 智选（每次启动都由算法决定）、手动（点卡片固定一个账号；
  再点一次回到智选，实时生效，连正在跑的会话也适用）、关闭（纯仪表盘，不介入启动）。
...
- **Status line 集成。** Claude Code 的 status line 会多一个紫色 ✦ Tally 信号（代表这个
  会话跑在 Tally 之下）与当前使用的账号名称。你原本自定义的 status line 会原封不动继续
  运行、只是后面多接一段信号；移除时逐字节还原成原样，就算你不卸载直接删掉 Tally
  也照常运行。
