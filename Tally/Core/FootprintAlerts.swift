...
struct FootprintAlertState: Equatable {
    ...
    /// turn starting, and the CPU's window is minutes rather than seconds
    /// (`FootprintAlarm.outlastsABuild`). There is no disk one, on purpose: a write rate has no
...
enum FootprintAlarm {
    ...
    static func advance(_ state: FootprintAlertState, reading: ProcessFootprint, idle: Bool,
                        at: Date, capacity: MachineCapacity = .current) -> FootprintAlertState {
