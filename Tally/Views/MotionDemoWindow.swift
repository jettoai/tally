...
private struct MotionDemoView: View {
    ...
    private var footer: some View {
        Text(verbatim: "The board runs whichever of these the launch asks for: "
             ...
             + "own is every motion off. Defaults are roll, grow, bouncy (N3 and L6). The line "
             + "stopped being a price when it stopped being the view tree's work: both axes are a "
             + "layer's now and neither lays the board out on every frame - see MotionChoice.lines "
             + "for what each one used to cost. N3v is the digits drawn the old way, in the view "
             + "tree, for the one difference a layer cannot spell: numericText's own blur.")
            .font(.caption2).foregroundStyle(.secondary)
    }
}
